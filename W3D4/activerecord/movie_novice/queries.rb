...
def below_average_years
  ...
  Movie.select(:yr, 'count(title)').where('score < 5').group_by(yr)
    .order_by('count(title) desc')
end
...
def alphabetized_actors
  ...
  Actor.select(:name).order_by('name').limit('10')
end
...
def pulp_fiction_actors
  ...
  Movie.select(:id, :name).join(:castings).on('actors.id = actor_id')
    .join(:movies).on('movie_id = movies.id').where('title = \'Pulp Fiction\'')
end
